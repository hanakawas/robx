-- GIR Filtreli Logger (Delta Android - Reentry-Safe)
local P=game:GetService("Players").LocalPlayer
local G=P:WaitForChild("PlayerGui")
local L={}
local IN_HOOK=false  -- reentry guard
...
-- REMOTE SPY (reentry-safe)
local old
old=hookmetamethod(game,"__namecall",function(self,...)
    if IN_HOOK then return old(self,...) end   -- iç içe girmeyi engelle
    local method
    local ok=true; ok,method=pcall(getnamecallmethod)
    if not ok or (method~="FireServer" and method~="InvokeServer") then
        return old(self,...)
    end
    -- IsA KULLANMA: typeof + ClassName (namecall tetiklemez)
    if typeof(self)~="Instance" then return old(self,...) end
    local cn=self.ClassName
    if cn~="RemoteEvent" and cn~="RemoteFunction" then
        return old(self,...)
    end
    IN_HOOK=true
    pcall(function()
...
-- UI BUTON (oyun tam yüklendikten sonra, ClassName ile)
task.spawn(function()
    if not game:IsLoaded() then game.Loaded:Wait() end
    task.wait(2)  -- GUI yerleşsin
...
t.Size=UDim2.new(1,0,0,22); t.Text="📋 LOGGER (Reentry-Safe)"
...
tb.Text="Hazir. Plant/Harvest/Sell yap..."; tb.Parent=f
